...
class AgendaApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sistema Médico - Agenda")
        ...
        self.status_options = ['Agendado', 'Confirmado', 'Cancelado', 'Realizado', 'Falta']
        self.dia_semana_options = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado', 'Domingo']
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(main_frame, text="Gerenciamento de Agenda", font=self.title_font)
        title_label.grid(row=0, column=0, columnspan=4, pady=(0, 20))

        search_frame = ttk.LabelFrame(main_frame, text=" Buscar Agendamentos ", padding=10)
        ...
        self.search_type = ttk.Combobox(search_frame, values=["ID", "Médico", "Data"], state="readonly")
        ...
        form_frame = ttk.LabelFrame(main_frame, text=" Dados do Agendamento ", padding=15)
        form_frame.grid(row=2, column=0, columnspan=4, sticky="nsew", pady=10)

        campos = [
            ("ID:", "id_entry", True),
            ("Data*:", "data_entry", False),
            ("Hora Início*:", "hora_inicio_entry", False),
            ("Hora Fim*:", "hora_fim_entry", False),
            ("Dia da Semana:", "dia_semana_combobox", False),
        ...
        for i, (text, attr, readonly) in enumerate(campos):
            ...
            if attr == "observacao_text":
                ...
                if "dia_semana" in attr:
                    values = self.dia_semana_options
                elif "status" in attr:
        ...
        self.status_combobox.set('Agendado')

        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=3, column=0, columnspan=4, pady=10)
        
        ttk.Button(button_frame, text="Novo", command=self.novo_agendamento).grid(row=0, column=0, padx=5)
        self.salvar_btn = ttk.Button(button_frame, text="Salvar", command=self.salvar_agendamento, state=tk.DISABLED)
        self.salvar_btn.grid(row=0, column=1, padx=5)
        self.atualizar_btn = ttk.Button(button_frame, text="Atualizar", command=self.atualizar_agendamento, state=tk.DISABLED)
        self.atualizar_btn.grid(row=0, column=2, padx=5)
        self.deletar_btn = ttk.Button(button_frame, text="Deletar", command=self.deletar_agendamento, state=tk.DISABLED)
        ...
        self.tree = ttk.Treeview(main_frame, 
                               columns=("ID", "Data", "Hora_Inicio", "Hora_Fim", "Dia_Semana", "Status", "Medico", "Observacao"), 
        ...
        self.tree.bind("<<TreeviewSelect>>", self.selecionar_agendamento)
    ...
    def buscar_por_tipo(self):
        ...
        try:
            if tipo == "ID":
                self.cursor.execute("SELECT * FROM Agenda WHERE ID = %s", (valor_busca,))
            elif tipo == "Médico":
                self.cursor.execute("""SELECT a.* FROM Agenda a
                                      JOIN Medico m ON a.Medico_ID = m.ID
                                      WHERE m.Nome LIKE %s""", (f"%{valor_busca}%",))
            else:  # Data
                try:
                    data_busca = datetime.strptime(valor_busca, '%d/%m/%Y').date()
                    self.cursor.execute("SELECT * FROM Agenda WHERE Data = %s", (data_busca,))
                except ValueError:
            ...
            if resultado:
                ...
                self.hora_inicio_entry.insert(0, resultado[2].strftime('%H:%M'))
                self.hora_fim_entry.delete(0, tk.END)
                self.hora_fim_entry.insert(0, resultado[3].strftime('%H:%M'))
                
                if resultado[4]:
                    self.dia_semana_combobox.set(resultado[4])
                
                self.status_combobox.set(resultado[5])
                
                # Carregar médico
                self.cursor.execute("SELECT CONCAT(ID, ' - ', Nome) FROM Medico WHERE ID = %s", (resultado[7],))
                medico = self.cursor.fetchone()[0]
                self.medico_combobox.set(medico)
                
                self.observacao_text.delete('1.0', tk.END)
                self.observacao_text.insert('1.0', resultado[6] if resultado[6] else "")
                
                self.atualizar_btn.config(state=tk.NORMAL)
                self.deletar_btn.config(state=tk.NORMAL)
                
                self.carregar_agendamentos(f"WHERE a.ID = {resultado[0]}")
            else:
                messagebox.showinfo("Informação", "Nenhum agendamento encontrado")
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao buscar: {str(e)}")

    def carregar_agendamentos(self, where_clause=""):
        self.tree.delete(*self.tree.get_children())
        try:
            query = f"""SELECT a.ID, a.Data, a.Hora_inicio, a.Hora_fim, a.Dia_semana, a.Status, 
                       CONCAT(m.ID, ' - ', m.Nome) AS Medico, a.Observacao
                       FROM Agenda a
                       JOIN Medico m ON a.Medico_ID = m.ID
                       {where_clause} 
            ...
            for row in self.cursor.fetchall():
                ...
                formatted_row[3] = row[3].strftime('%H:%M')
    ...
    def selecionar_agendamento(self, event):
        ...
        if item:
            ...
            self.hora_inicio_entry.delete(0, tk.END)
            self.hora_inicio_entry.insert(0, dados[2])
            self.hora_fim_entry.delete(0, tk.END)
            self.hora_fim_entry.insert(0, dados[3])
            if len(dados) > 4 and dados[4]:
                self.dia_semana_combobox.set(dados[4])
            self.status_combobox.set(dados[5])
            self.medico_combobox.set(dados[6])
            self.observacao_text.delete('1.0', tk.END)
            if len(dados) > 7 and dados[7]:
                self.observacao_text.insert('1.0', dados[7])
            self.atualizar_btn.config(state=tk.NORMAL)
            self.deletar_btn.config(state=tk.NORMAL)
    ...
    def validar_campos(self):
        ...
        # Validar horas
        try:
            hora_inicio = datetime.strptime(self.hora_inicio_entry.get(), '%H:%M').time()
            hora_fim = datetime.strptime(self.hora_fim_entry.get(), '%H:%M').time()
            
            if hora_fim <= hora_inicio:
                messagebox.showerror("Erro", "Hora fim deve ser depois da hora início!")
                self.hora_fim_entry.focus()
                return False
        except ValueError:
            messagebox.showerror("Erro", "Formato de hora inválido! Use HH:MM")
            self.hora_inicio_entry.focus()
            return False
    ...
    def salvar_agendamento(self):
        ...
        try:
            ...
            hora_inicio = datetime.strptime(self.hora_inicio_entry.get(), '%H:%M').time()
            hora_fim = datetime.strptime(self.hora_fim_entry.get(), '%H:%M').time()
            dia_semana = self.dia_semana_combobox.get() or None
            
            query = """INSERT INTO Agenda 
                      (Data, Hora_inicio, Hora_fim, Dia_semana, Status, Observacao, Medico_ID) 
    ...
    def deletar_agendamento(self):
        if not self.id_entry.get():
            messagebox.showerror("Erro", "Selecione um agendamento para deletar")
            return

        if not messagebox.askyesno("Confirmar", "Tem certeza que deseja deletar este agendamento?"):
            return

        try:
            self.cursor.execute("DELETE FROM Agenda WHERE ID = %s", (self.id_entry.get(),))
            self.conexao.commit()
