...
class ConsultorioApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sistema Médico - Consultórios")
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(main_frame, text="Gerenciamento de Consultórios", font=self.title_font)
        title_label.grid(row=0, column=0, columnspan=4, pady=(0, 20))

        search_frame = ttk.LabelFrame(main_frame, text=" Buscar Consultórios ", padding=10)
        ...
        self.search_type = ttk.Combobox(search_frame, values=["ID", "Nome"], state="readonly")
        ...
        form_frame = ttk.LabelFrame(main_frame, text=" Dados do Consultório ", padding=15)
        ...
        ttk.Button(button_frame, text="Novo", command=self.novo_consultorio).grid(row=0, column=0, padx=5)
        self.salvar_btn = ttk.Button(button_frame, text="Salvar", command=self.salvar_consultorio, state=tk.DISABLED)
        self.salvar_btn.grid(row=0, column=1, padx=5)
        self.atualizar_btn = ttk.Button(button_frame, text="Atualizar", command=self.atualizar_consultorio, state=tk.DISABLED)
        self.atualizar_btn.grid(row=0, column=2, padx=5)
        self.deletar_btn = ttk.Button(button_frame, text="Deletar", command=self.deletar_consultorio, state=tk.DISABLED)
        ...
        self.tree = ttk.Treeview(main_frame, 
                               columns=("ID", "Nome", "Endereco_ID"), 
                               show="headings", height=12)
        self.tree.grid(row=4, column=0, columnspan=4, sticky="nsew", pady=10)
        
        for col in ("ID", "Nome", "Endereco_ID"):
            self.tree.heading(col, text=col)
            self.tree.column(col, width=100, anchor="w")
        
        self.tree.column("Nome", width=250)
        
        self.tree.bind("<<TreeviewSelect>>", self.selecionar_consultorio)
        self.carregar_consultorios()
    ...
    def buscar_por_tipo(self):
        ...
        try:
            if tipo == "ID":
                self.cursor.execute("SELECT * FROM Consultorio WHERE ID = %s", (valor_busca,))
            else:
                self.cursor.execute("SELECT * FROM Consultorio WHERE Nome LIKE %s", (f"%{valor_busca}%",))
            ...
            if resultado:
                ...
                self.endereco_id_entry.insert(0, resultado[2] if resultado[2] else "")
                
                self.atualizar_btn.config(state=tk.NORMAL)
                self.deletar_btn.config(state=tk.NORMAL)
                
                self.carregar_consultorios(f"WHERE ID = {resultado[0]}")
            else:
                messagebox.showinfo("Informação", "Nenhum consultório encontrado")
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao buscar: {str(e)}")

    def carregar_consultorios(self, where_clause=""):
        self.tree.delete(*self.tree.get_children())
        try:
            query = f"SELECT ID, Nome, Endereco_ID FROM Consultorio {where_clause} ORDER BY ID"
    ...
    def selecionar_consultorio(self, event):
        ...
        if item:
            ...
            self.endereco_id_entry.insert(0, dados[2] if dados[2] else "")
            self.atualizar_btn.config(state=tk.NORMAL)
            self.deletar_btn.config(state=tk.NORMAL)

    def novo_consultorio(self):
        self.limpar_campos()
    ...
    def salvar_consultorio(self):
        ...
        try:
            ...
            self.conexao.commit()
            messagebox.showinfo("Sucesso", "Consultório cadastrado com sucesso!")
            self.limpar_campos()
            self.carregar_consultorios()
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao salvar: {str(e)}")

    def atualizar_consultorio(self):
        if not self.id_entry.get():
            messagebox.showerror("Erro", "Selecione um consultório para atualizar")
            return
        ...
        try:
            ...
            self.cursor.execute(query, valores)
            self.conexao.commit()
            messagebox.showinfo("Sucesso", "Consultório atualizado com sucesso!")
            self.carregar_consultorios()
        except Exception as e:
            messagebox.showerror("Erro", f"Falha ao atualizar: {str(e)}")

    def deletar_consultorio(self):
        if not self.id_entry.get():
            messagebox.showerror("Erro", "Selecione um consultório para deletar")
            return

        if not messagebox.askyesno("Confirmar", "Tem certeza que deseja deletar este consultório?"):
            return

        try:
            self.cursor.execute("DELETE FROM Consultorio WHERE ID = %s", (self.id_entry.get(),))
            self.conexao.commit()
            messagebox.showinfo("Sucesso", "Consultório deletado com sucesso!")
            self.limpar_campos()
            self.carregar_consultorios()
        except Exception as e:
